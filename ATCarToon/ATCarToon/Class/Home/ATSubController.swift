...
class ATSubController: BaseConnectionController {
    ...
    override func refreshData(page: Int) {
        ATMoya.apiMoya(target: .apiSub, sucesss: { (json) in
            if page == RefreshPageStart{
                self.listData.removeAll();
            }
            var datas : [ATHomeInfo] = [];
            if let list = [ATHomeInfo].deserialize(from: json["newSubscribeList"].rawString()){
